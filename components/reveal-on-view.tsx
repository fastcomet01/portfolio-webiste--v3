...
export default function RevealOnView({ as = "div", className, children, delay = 0, style, intensity = "soft", staggerChildren = false }: RevealOnViewProps) {
  ...
  useEffect(() => {
    ...
    const cleanup = inView(element, () => {
      const targetsRaw = staggerChildren
        ? Array.from(element.children) as HTMLElement[]
        : [element]

      // Exclude overlay/background elements from animation
      const targets = staggerChildren
        ? targetsRaw.filter((t) =>
            !t.hasAttribute("aria-hidden") &&
            !t.classList.contains("pointer-events-none") &&
            !t.classList.contains("absolute") &&
            !t.hasAttribute("data-no-animate")
          )
        : targetsRaw
      ...
      targets.forEach((target, index) => {
        const targetDelay = targets.length > 1 ? delay + (index * 0.12) : delay
